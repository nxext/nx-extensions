...
export function getPublishedVersion(): string {
  process.env.PUBLISHED_VERSION =
    ...
    readJsonFile(`./dist/packages/vitest/package.json`).version ||
...
export function getNxVersion(): string {
  return (
    readJsonFile(`./package.json`).devDependencies['@nrwl/workspace'] ||
    'latest'
  );
}
