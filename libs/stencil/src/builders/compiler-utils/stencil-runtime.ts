...
export function createStencilConfig(
  ...
  return from(
    ...
    map((loadConfigResults) => loadConfigResults.config)
  );
}
...
export function parseRunParameters(runOptions: string[], options: StencilBuildOptions | StencilTestOptions | StencilE2EOptions) {
