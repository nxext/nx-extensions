...
export function addProject(tree: Tree, options: NormalizedSchema) {
  ...
  addProjectPackageJson(tree, {
    ...
    // (`addVite`, called after this - it already branches on TS-solution
    // internally). Non-buildable, non-publishable libs never go through
    // that generator though, and in TS-solution mode every project still
    // needs a resolvable entry point for other workspace projects that
    // consume it via the package-manager symlink (no separate build step) -
    // mirrors @nx/js's `determineEntryFields` for `bundler: 'none'`.
