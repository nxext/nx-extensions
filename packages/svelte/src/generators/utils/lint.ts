 ...
 * ESM-only packages that ship flat-config exports exclusively (unlike the
 * old, now-dead `eslint-plugin-svelte3`, which used a legacy `.eslintrc`
 * processor). There is no legacy-`.eslintrc`-compatible shareable config to
 * `extends` from any more, so if the workspace's own ESLint config is still
 * in legacy format (ESLint < 9 without `ESLINT_USE_FLAT_CONFIG=true`),
 * wiring the plugin into a legacy config would crash at lint time with
 * `ERR_REQUIRE_ESM`. In that case this is a no-op: `.ts` files keep linting
 * as usual via whatever `lintProjectGenerator` already set up, `.svelte`
 * files are just not covered by Svelte-specific rules until the workspace
 * migrates to flat config.
 */
