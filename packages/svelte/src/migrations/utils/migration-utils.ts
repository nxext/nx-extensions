...
export function isSvelteLegacyTargetProject(
  project: any
): boolean {
...
export function isSvelteProject(
  project: any
): boolean {
  return ['build', 'test', 'serve', 'e2e'].map(command => {
    const target =
    ...
    return (
      target &&
      (
        ((target.executor === `@nxext/vite:${command}` || target.executor === `@nxext/vite:package`) && target?.options?.frameworkConfigFile.startsWith('@nxext/svelte'))
        || target.executor === `@nxext/svelte:${command}`
      )
    );
  }).some(value => value === true);
