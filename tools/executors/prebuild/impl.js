...
exports["default"] = architect_1.createBuilder(function (options, context) {
    ...
    return rxjs_1.concat(dependencies.map(function (dep) {
        ...
        return rxjs_1.from(architect_1.scheduleTargetAndForget(context, architect_1.targetFromTargetString(targetProject + ":" + options.target)))
            .pipe(operators_1.tap(function () { return context.logger.info("Building @" + npmScope + "/" + targetProject + "..."); }));
    }), operators_1.tap(function () {
        var publishableLibNames = getPublishableLibNames();
        var packageJsonFilePath = getProjectDistPath(context, targetProject) + "/package.json";
...
function getProjectDistPath(context, name) {
